...
class AndroidViewer(ControlMixin):
    ...
    def __init__(self, max_width=0, bitrate=8000000, max_fps=30, adb_path='E:\\pythonProject\\adb',
    ...
    def init_server_connection(self):
        ...
        while True:
            raw_h264 = self.video_socket.recv(64)
            print(111)
            print(raw_h264)
            sleep(1)
    ...
    def deploy_server(self, max_width=1024, bitrate=8000000, max_fps=0):
        try:
            ...
            # adb_push = subprocess.Popen([self.adb_path, 'push', server_file_path, '/sdcard/data/local/tmp/'],
            #                             stdout=subprocess.PIPE, shell=True, stderr=subprocess.PIPE, cwd=server_root)
            command_push = 'adb push ' + server_file_path + ' ' + '/sdcard/data/local/tmp/'# 获取连接设备
            adb_push = subprocess.Popen(command_push, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            # adb_push_comm = ''.join([x.decode("GBK") for x in adb_push.communicate() if x is not None])
            # if "error" in adb_push_comm:
            #     logger.critical("Is your device/emulator visible to ADB?")
            #     raise Exception(adb_push_comm)

            logger.info("Running server...")
            command_process = 'adb shell CLASSPATH=/sdcard/data/local/tmp/scrcpy-server.jar app_process /sdcard ' + \
                              'com.genymobile.scrcpy.Server 1.12.1 1024 8000000 25 true - false true'
            subprocess.Popen(command_process, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # subprocess.Popen(
            #     [self.adb_path, 'shell',
            #      'CLASSPATH=/sdcard/data/local/tmp/scrcpy-server.jar',
            #      'app_process', '/', 'com.genymobile.scrcpy.Server 1.12.1 {} {} {} true - false true'.format(
            #         max_width, bitrate, max_fps)],
            #     shell=True, cwd=server_root)
            sleep(1)

            logger.info("Forward server port...")
            command_forward = 'adb forward tcp:8081 localabstract:scrcpy'
            subprocess.Popen(command_forward, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # subprocess.Popen(
            #     [self.adb_path, 'forward', 'tcp:8081', 'localabstract:scrcpy'],
            #     shell=True, cwd=server_root).wait()
            sleep(2)
